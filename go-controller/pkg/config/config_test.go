...
var _ = Describe("Config Operations", func() {
	...
	It("uses expected defaults", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			for _, a := range []*OvnDBAuth{OvnNorth.ClientAuth, OvnSouth.ClientAuth} {
				...
				Expect(a.OvnAddressForServer).To(Equal(""))
			}
	...
	It("reads defaults from ovs-vsctl external IDs", func() {
		app.Action = func(ctx *cli.Context) error {
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:k8s-api-server", Output: "https://somewhere.com:8081"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:k8s-api-token", Output: "asadfasdfasrw3atr3r3rf33fasdaa3233"})
			fname, err := createTempFile("kube-cacert.pem")
			Expect(err).NotTo(HaveOccurred())
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:k8s-ca-certificate", Output: fname})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:ovn-nb", Output: "tcp:1.1.1.1:6441"})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			cfgPath, err := InitConfig(ctx, fexec, &Defaults{OvnNorthAddress: true, K8sAPIServer: true, K8sToken: true, K8sCert: true})
			Expect(err).NotTo(HaveOccurred())
			Expect(cfgPath).To(Equal(cfgFile.Name()))
			...
			Expect(OvnNorth.ClientAuth.OvnAddressForServer).To(Equal(""))
			...
			Expect(OvnSouth.ClientAuth.OvnAddressForServer).To(Equal(""))
			...
			Expect(OvnNorth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnNorth.ServerAuth.OvnAddressForServer).To(Equal("ptcp:6441"))
			...
			Expect(OvnSouth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnSouth.ServerAuth.OvnAddressForServer).To(Equal(""))
			return nil
		}
		err := app.Run([]string{app.Name, "-config-file=" + cfgFile.Name()})
		Expect(err).NotTo(HaveOccurred())
	...
	It("reads defaults (multiple master) from ovs-vsctl external IDs", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			Expect(err).NotTo(HaveOccurred())
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:k8s-ca-certificate", Output: fname})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 --if-exists get Open_vSwitch . external_ids:ovn-nb", Output: "tcp:1.1.1.1:6441,tcp:1.1.1.2:6641,tcp:1.1.1.3:6641"})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			cfgPath, err := InitConfig(ctx, fexec, &Defaults{OvnNorthAddress: true, K8sAPIServer: true, K8sToken: true, K8sCert: true})
			...
			Expect(OvnNorth.ClientAuth.CACert).To(Equal(""))
			Expect(OvnNorth.ClientAuth.server).To(BeFalse())
			Expect(OvnNorth.ClientAuth.OvnAddressForClient).To(Equal("tcp:1.1.1.1:6441,tcp:1.1.1.2:6641,tcp:1.1.1.3:6641"))
			Expect(OvnNorth.ClientAuth.OvnAddressForServer).To(Equal(""))
			Expect(OvnSouth.ClientAuth.Scheme).To(Equal(OvnDBSchemeUnix))
			Expect(OvnSouth.ClientAuth.PrivKey).To(Equal(""))
			Expect(OvnSouth.ClientAuth.Cert).To(Equal(""))
...
server-cacert=%s`, kubeconfigFile, kubeCAFile, nbServerCAFile, sbServerCAFile)
		...
		app.Action = func(ctx *cli.Context) error {
			...
			Expect(OvnNorth.ServerAuth.server).To(BeTrue())
			Expect(OvnNorth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnNorth.ServerAuth.OvnAddressForServer).To(Equal("pssl:6641"))
			...
			Expect(OvnSouth.ServerAuth.server).To(BeTrue())
			Expect(OvnSouth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnSouth.ServerAuth.OvnAddressForServer).To(Equal("pssl:6642"))
			return nil
		}
		err = app.Run([]string{app.Name, "-config-file=" + cfgFile.Name()})
		Expect(err).NotTo(HaveOccurred())
...
server-cacert=/path/to/sb-ca.crt`), 0644)
		...
		app.Action = func(ctx *cli.Context) error {
			...
			Expect(OvnNorth.ServerAuth.server).To(BeTrue())
			Expect(OvnNorth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnNorth.ServerAuth.OvnAddressForServer).To(Equal("pssl:6651"))
			Expect(OvnSouth.ClientAuth.Scheme).To(Equal(OvnDBSchemeSSL))
			...
			Expect(OvnSouth.ServerAuth.server).To(BeTrue())
			Expect(OvnSouth.ServerAuth.OvnAddressForClient).To(Equal(""))
			Expect(OvnSouth.ServerAuth.OvnAddressForServer).To(Equal("pssl:6652"))
			return nil
		}
		cliArgs := []string{app.Name, "-config-file=" + cfgFile.Name(), "-mtu=1234", "-conntrack-zone=5555", "-loglevel=3", "-logfile=/some/logfile", "-cni-conf-dir=/some/cni/dir", "-cni-plugin=a-plugin", "-k8s-kubeconfig=" + kubeconfigFile, "-k8s-apiserver=https://4.4.3.2:8080", "-k8s-cacert=" + kubeCAFile, "-k8s-token=asdfasdfasdfasfd", "-nb-address=ssl://6.5.4.3:6651", "-nb-server-privkey=/server/privkey", "-nb-server-cert=/server/cert", "-nb-server-cacert=" + nbServerCAFile, "-nb-client-privkey=/client/privkey", "-nb-client-cert=/client/cert", "-nb-client-cacert=/client/cacert", "-sb-address=ssl://6.5.4.1:6652", "-sb-server-privkey=/server/privkey2", "-sb-server-cert=/server/cert2", "-sb-server-cacert=" + sbServerCAFile, "-sb-client-privkey=/client/privkey2", "-sb-client-cert=/client/cert2", "-sb-client-cacert=/client/cacert2"}
		err = app.Run(cliArgs)
		Expect(err).NotTo(HaveOccurred())
	})
...
server-cacert=/path/to/sb-ca.crt`), 0644)
		...
		app.Action = func(ctx *cli.Context) error {
			...
			Expect(OvnNorth.ClientAuth.CACert).To(Equal("/client/cacert"))
			Expect(OvnNorth.ClientAuth.server).To(BeFalse())
			Expect(OvnNorth.ClientAuth.OvnAddressForClient).To(Equal("ssl:6.5.4.3:6651,ssl:6.5.4.4:6651,ssl:6.5.4.5:6651"))
			Expect(OvnNorth.ClientAuth.OvnAddressForServer).To(Equal(""))
			Expect(OvnNorth.ServerAuth.Scheme).To(Equal(OvnDBSchemeSSL))
			Expect(OvnNorth.ServerAuth.PrivKey).To(Equal("/server/privkey"))
			Expect(OvnNorth.ServerAuth.Cert).To(Equal("/server/cert"))
			...
			Expect(OvnSouth.ClientAuth.CACert).To(Equal("/client/cacert2"))
			Expect(OvnSouth.ClientAuth.server).To(BeFalse())
			Expect(OvnSouth.ClientAuth.OvnAddressForClient).To(Equal("ssl:6.5.4.1:6652,ssl:6.5.4.2:6652,ssl:6.5.4.3:6652"))
			Expect(OvnSouth.ClientAuth.OvnAddressForServer).To(Equal(""))
			Expect(OvnSouth.ServerAuth.Scheme).To(Equal(OvnDBSchemeSSL))
			Expect(OvnSouth.ServerAuth.PrivKey).To(Equal("/server/privkey2"))
			Expect(OvnSouth.ServerAuth.Cert).To(Equal("/server/cert2"))
		...
		cliArgs := []string{app.Name, "-config-file=" + cfgFile.Name(), "-mtu=1234", "-conntrack-zone=5555", "-loglevel=3", "-logfile=/some/logfile", "-cni-conf-dir=/some/cni/dir", "-cni-plugin=a-plugin", "-k8s-kubeconfig=" + kubeconfigFile, "-k8s-apiserver=https://4.4.3.2:8080", "-k8s-cacert=" + kubeCAFile, "-k8s-token=asdfasdfasdfasfd", "-nb-address=ssl://6.5.4.3:6651,ssl://6.5.4.4:6651,ssl://6.5.4.5:6651", "-nb-server-privkey=/server/privkey", "-nb-server-cert=/server/cert", "-nb-server-cacert=" + nbServerCAFile, "-nb-client-privkey=/client/privkey", "-nb-client-cert=/client/cert", "-nb-client-cacert=/client/cacert", "-sb-address=ssl://6.5.4.1:6652,ssl://6.5.4.2:6652,ssl://6.5.4.3:6652", "-sb-server-privkey=/server/privkey2", "-sb-server-cert=/server/cert2", "-sb-server-cacert=" + sbServerCAFile, "-sb-client-privkey=/client/privkey2", "-sb-client-cert=/client/cert2", "-sb-client-cacert=/client/cacert2"}
		err = app.Run(cliArgs)
		Expect(err).NotTo(HaveOccurred())
	})
	Describe("OvnDBAuth operations", func() {
		...
		It("configures server northbound SSL correctly", func() {
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{Cmd: "ovn-nbctl set-connection pssl:6641 -- set connection . inactivity_probe=0"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovn-nbctl del-ssl"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovn-nbctl set-ssl %s %s %s", keyFile, certFile, caFile)})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			var err error
			...
			Expect(err).NotTo(HaveOccurred())
			a, err := newOvnDBAuth(fexec, "ovn-nbctl", "ovn-nb", nbURL, keyFile, certFile, caFile, true)
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(a.server).To(BeTrue())
			Expect(a.OvnAddressForClient).To(Equal(""))
			Expect(a.OvnAddressForServer).To(Equal("pssl:6641"))
			Expect(a.ctlCmd).To(Equal("ovn-nbctl"))
			Expect(a.externalID).To(Equal("ovn-nb"))
			Expect(a.GetURL()).To(Equal("pssl:6641"))
			err = a.SetDBAuth()
			Expect(err).NotTo(HaveOccurred())
			Expect(fexec.CommandCalls).To(Equal(len(fakeCmds)))
		})
		It("configures client northbound SSL correctly", func() {
			...
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovn-nbctl --db=%s --timeout=5 --private-key=%s --certificate=%s --bootstrap-ca-cert=%s list nb_global", nbURLOVN, keyFile, certFile, caFile)})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovs-vsctl --timeout=15 set Open_vSwitch . external_ids:ovn-nb=\"%s\"", nbURLOVN)})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			a, err := newOvnDBAuth(fexec, "ovn-nbctl", "ovn-nb", nbURL, keyFile, certFile, caFile, false)
			Expect(err).NotTo(HaveOccurred())
			Expect(a.Scheme).To(Equal(OvnDBSchemeSSL))
			Expect(a.PrivKey).To(Equal(keyFile))
			Expect(a.Cert).To(Equal(certFile))
		...
		It("configures server southbound SSL correctly", func() {
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{Cmd: "ovn-sbctl set-connection pssl:6642 -- set connection . inactivity_probe=0"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovn-sbctl del-ssl"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovn-sbctl set-ssl %s %s %s", keyFile, certFile, caFile)})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			var err error
			caFile, err = createTempFile("ca.crt")
			Expect(err).NotTo(HaveOccurred())
			a, err := newOvnDBAuth(fexec, "ovn-sbctl", "ovn-remote", sbURL, keyFile, certFile, caFile, true)
			Expect(err).NotTo(HaveOccurred())
			Expect(a.Scheme).To(Equal(OvnDBSchemeSSL))
			Expect(a.PrivKey).To(Equal(keyFile))
			Expect(a.Cert).To(Equal(certFile))
			...
			Expect(a.server).To(BeTrue())
			Expect(a.OvnAddressForClient).To(Equal(""))
			Expect(a.OvnAddressForServer).To(Equal("pssl:6642"))
			Expect(a.ctlCmd).To(Equal("ovn-sbctl"))
			Expect(a.externalID).To(Equal("ovn-remote"))
			Expect(a.GetURL()).To(Equal("pssl:6642"))
			err = a.SetDBAuth()
			Expect(err).NotTo(HaveOccurred())
			Expect(fexec.CommandCalls).To(Equal(len(fakeCmds)))
		})
		It("configures client southbound SSL correctly", func() {
			...
			fakeCmds := ovntest.AddFakeCmd(nil, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovn-nbctl --db=%s --timeout=5 --private-key=%s --certificate=%s --bootstrap-ca-cert=%s list nb_global", sbURLOVN, keyFile, certFile, caFile)})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: "ovs-vsctl --timeout=15 del-ssl"})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovs-vsctl --timeout=15 set-ssl %s %s %s", keyFile, certFile, caFile)})
			fakeCmds = ovntest.AddFakeCmd(fakeCmds, &ovntest.ExpectedCmd{Cmd: fmt.Sprintf("ovs-vsctl --timeout=15 set Open_vSwitch . external_ids:ovn-remote=\"%s\"", sbURLOVN)})
			fexec := &fakeexec.FakeExec{CommandScript: fakeCmds, LookPathFunc: func(file string) (string, error) {
				return fmt.Sprintf("/fake-bin/%s", file), nil
			}}
			a, err := newOvnDBAuth(fexec, "ovn-sbctl", "ovn-remote", sbURL, keyFile, certFile, caFile, false)
			Expect(err).NotTo(HaveOccurred())
			Expect(a.Scheme).To(Equal(OvnDBSchemeSSL))
			Expect(a.PrivKey).To(Equal(keyFile))
			Expect(a.Cert).To(Equal(certFile))
	...
	Describe("Kubernetes config options", func() {
		Context("returns an error when the", func() {
			generateTestsSimple("CA cert does not exist", "kubernetes CA certificate file \"/foo/bar/baz.cert\" not found", "-k8s-apiserver=https://localhost:8443", "-k8s-cacert=/foo/bar/baz.cert")
			generateTestsSimple("apiserver URL scheme is invalid", "kubernetes API server URL scheme \"gggggg\" invalid", "-k8s-apiserver=gggggg://localhost:8443")
			generateTestsSimple("apiserver URL is invalid", "kubernetes API server address \"http://a b.com/\" invalid: parse http://a b.com/: invalid character \" \" in host name", "-k8s-apiserver=http://a b.com/")
			generateTestsSimple("kubeconfig file does not exist", "kubernetes kubeconfig file \"/foo/bar/baz\" not found", "-k8s-kubeconfig=/foo/bar/baz")
		})
	})
	Describe("OVN API config options", func() {
		...
		Context("returns an error when", func() {
			generateTests("the scheme is not empty/tcp/ssl", "unknown OVN DB scheme \"blah\"", func() []string {
				return []string{"address=blah://1.2.3.4:5555"}
			})
			generateTests("the address is unix socket and certs are given", "certificate or key given; perhaps you mean to use the 'ssl' scheme?", func() []string {
				return []string{"client-privkey=/bar/baz/foo", "client-cert=/bar/baz/foo", "client-cacert=/var/baz/foo"}
			})
			generateTests("the OVN URL has no port", "Failed to parse OVN address tcp:4.3.2.1", func() []string {
				return []string{"address=tcp://4.3.2.1"}
			})
			generateTests("the OVN URL is a hostname", "OVN DB host \"foobar.org:4444\" must be an IP address, not a DNS name", func() []string {
				return []string{"address=tcp://foobar.org:4444"}
			})
			generateTests("certs are provided for the TCP scheme", "certificate or key given; perhaps you mean to use the 'ssl' scheme?", func() []string {
				return []string{"address=tcp://1.2.3.4:444", "client-privkey=/bar/baz/foo"}
			})
		})
		Context("does not return an error when", func() {
			generateTests("the SSL scheme is missing a client CA cert", "", func() []string {
				return []string{"address=ssl://1.2.3.4:444", "client-privkey=" + keyFile, "client-cert=" + certFile, "client-cacert=/foo/bar/baz"}
			})
			generateTests("the SSL scheme is missing a private key file", "", func() []string {
				return []string{"address=ssl://1.2.3.4:444", "client-privkey=/foo/bar/baz", "client-cert=" + certFile, "client-cacert=" + caFile}
			})
			generateTests("the SSL scheme is missing a client cert file", "", func() []string {
				return []string{"address=ssl://1.2.3.4:444", "client-privkey=" + keyFile, "client-cert=/foo/bar/baz", "client-cacert=" + caFile}
			})
		})
	})
})
