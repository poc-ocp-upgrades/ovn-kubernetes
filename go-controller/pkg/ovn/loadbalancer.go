...
func (ovn *Controller) getLoadBalancer(protocol kapi.Protocol) (string, error) {
	...
	if protocol == kapi.ProtocolTCP {
		out, _, err = util.RunOVNNbctl("--data=bare", "--no-heading", "--columns=_uuid", "find", "load_balancer", "external_ids:k8s-cluster-lb-tcp=yes")
	} else if protocol == kapi.ProtocolUDP {
		out, _, err = util.RunOVNNbctl("--data=bare", "--no-heading", "--columns=_uuid", "find", "load_balancer", "external_ids:k8s-cluster-lb-udp=yes")
	}
	if err != nil {
		return "", err
	}
...
func (ovn *Controller) getDefaultGatewayLoadBalancer(protocol kapi.Protocol) string {
	...
	var gw string
	gw, _, _ = util.RunOVNNbctl("--data=bare", "--no-heading", "--columns=name", "find", "logical_router", "options:lb_force_snat_ip=100.64.1.2")
	if len(gw) == 0 {
		logrus.Errorf("Error locating default gateway")
		return ""
	}
	externalIDKey := string(protocol) + "_lb_gateway_router"
	lb, _, _ := util.RunOVNNbctl("--data=bare", "--no-heading", "--columns=_uuid", "find", "load_balancer", "external_ids:"+externalIDKey+"="+gw)
	if len(lb) != 0 {
...
func (ovn *Controller) deleteLoadBalancerVIP(loadBalancer, vip string) {
	...
	if err != nil {
		logrus.Errorf("Error in deleting load balancer vip %s for %s"+"stdout: %q, stderr: %q, error: %v", vip, loadBalancer, stdout, stderr, err)
	}
}
func (ovn *Controller) createLoadBalancerVIP(lb string, serviceIP string, port int32, ips []string, targetPort int32) error {
	...
	if len(ips) == 0 {
		_, _, err := util.RunOVNNbctl("remove", "load_balancer", lb, "vips", key)
		return err
	}
	...
	if err != nil {
		logrus.Errorf("Error in creating load balancer: %s "+"stdout: %q, stderr: %q, error: %v", lb, out, stderr, err)
	}
	return err
}
