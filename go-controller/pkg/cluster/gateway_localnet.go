...
func generateGatewayNATRules(ifname string, ip string) []iptRule {
	...
	rules = append(rules, iptRule{table: "filter", chain: "INPUT", args: []string{"-i", ifname, "-m", "comment", "--comment", "from OVN to localhost", "-j", "ACCEPT"}})
	rules = append(rules, iptRule{table: "nat", chain: "POSTROUTING", args: []string{"-s", ip, "-j", "MASQUERADE"}})
	return rules
}
func localnetGatewayNAT(ipt util.IPTablesHelper, ifname, ip string) error {
	...
	for _, r := range rules {
		...
		if err != nil {
			return fmt.Errorf("failed to add iptables %s/%s rule %q: %v", r.table, r.chain, strings.Join(r.args, " "), err)
		}
	}
	return nil
}
...
func initLocalnetGatewayInternal(nodeName string, clusterIPSubnet []string, subnet string, ipt util.IPTablesHelper, nodePortEnable bool) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to create localnet bridge %s"+", stderr:%s (%v)", localnetBridgeName, stderr, err)
	}
	...
	_, stderr, err = util.RunOVSVsctl("--may-exist", "add-port", localnetBridgeName, localnetBridgeNextHop, "--", "set", "interface", localnetBridgeNextHop, "type=internal")
	if err != nil {
		return fmt.Errorf("Failed to create localnet bridge next hop %s"+", stderr:%s (%v)", localnetBridgeNextHop, stderr, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to flush ip address of %s (%v)", localnetBridgeNextHop, err)
	}
	_, _, err = util.RunIP("addr", "add", localnetGatewayNextHopSubnet, "dev", localnetBridgeNextHop)
	if err != nil {
		return fmt.Errorf("failed to assign ip address to %s (%v)", localnetBridgeNextHop, err)
	}
	err = util.GatewayInit(clusterIPSubnet, nodeName, localnetGatewayIP, "", localnetBridgeName, localnetGatewayNextHop, subnet, nodePortEnable)
	if err != nil {
