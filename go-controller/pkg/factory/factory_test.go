...
var _ = Describe("Watch Factory Operations", func() {
	...
	Context("when a processExisting is given", func() {
		testExisting := func(objType reflect.Type, namespace string, lsel *metav1.LabelSelector) {
			...
			h, err := wf.addHandler(objType, namespace, lsel, cache.ResourceEventHandlerFuncs{}, func(objs []interface{}) {
		...
		It("is called for each existing pod that matches a given namespace and label", func() {
			...
			testExisting(podType, "default", &metav1.LabelSelector{MatchLabels: map[string]string{"blah": "foobar"}})
		})
	})
	Context("when existing items are known to the informer", func() {
		testExisting := func(objType reflect.Type) {
			wf, err := NewWatchFactory(fakeClient, stop)
			Expect(err).NotTo(HaveOccurred())
			h, err := wf.addHandler(objType, "", nil, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
				numAdded++
	...
	addFilteredHandler := func(wf *WatchFactory, objType reflect.Type, namespace string, lsel *metav1.LabelSelector, funcs cache.ResourceEventHandlerFuncs) *Handler {
		h, err := wf.addHandler(objType, namespace, lsel, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			defer GinkgoRecover()
			numAdded++
			funcs.AddFunc(obj)
		}, UpdateFunc: func(old, new interface{}) {
	...
	It("responds to pod add/update/delete events", func() {
		...
		h := addHandler(wf, podType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			pod := obj.(*v1.Pod)
	...
	It("responds to namespace add/update/delete events", func() {
		...
		h := addHandler(wf, namespaceType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			ns := obj.(*v1.Namespace)
	...
	It("responds to node add/update/delete events", func() {
		...
		h := addHandler(wf, nodeType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			node := obj.(*v1.Node)
	...
	It("responds to policy add/update/delete events", func() {
		...
		h := addHandler(wf, policyType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
	...
	It("responds to endpoints add/update/delete events", func() {
		...
		h := addHandler(wf, endpointsType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
		...
		added.Subsets = append(added.Subsets, v1.EndpointSubset{Ports: []v1.EndpointPort{{Name: "foobar", Port: 1234}}})
	...
	It("responds to service add/update/delete events", func() {
		...
		h := addHandler(wf, serviceType, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
	...
	It("filters correctly by label and namespace", func() {
		...
		addFilteredHandler(wf, podType, "default", &metav1.LabelSelector{MatchLabels: map[string]string{"blah": "foobar"}}, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			pod := obj.(*v1.Pod)
	...
	It("correctly handles object updates that cause filter changes", func() {
		...
		equalPod := pod
		h := addFilteredHandler(wf, podType, "default", &metav1.LabelSelector{MatchLabels: map[string]string{"blah": "foobar"}}, cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
			p := obj.(*v1.Pod)
